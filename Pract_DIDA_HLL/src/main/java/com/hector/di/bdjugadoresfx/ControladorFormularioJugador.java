...
public class ControladorFormularioJugador extends ControladorConNavegabilidad implements Initializable{
    ...
    public void guardar(){
        if(!isInteger(goles.getText())){
            ...
            alert.setHeaderText("Tipo de dato no válido");
            alert.setContentText("Introduce un número entero en el campo 'goles'");
            alert.show();
       }
        else if(!isDouble(nota.getText())){
            Alert alert = new Alert(AlertType.INFORMATION); 
            alert.setTitle("Error");
            alert.setHeaderText("Tipo de dato no válido");
            alert.setContentText("Introduce un número en el campo 'nota' (entre 0 y 10)");
            alert.show();
       }
       ...
       if (!portero.isSelected() && !defensa.isSelected() && !centrocampista.isSelected() && !delantero.isSelected()){
            Alert alert = new Alert(AlertType.INFORMATION);
            alert.setTitle("Error");
            alert.setHeaderText("Posición vacía");
            alert.setContentText("Por favor, seleccione una posición válida");
            alert.show();
            
       }//Lo mismo para el club
       else if(combobox2.getValue() == null || combobox2.getValue().equals("Club")){
            Alert alert = new Alert(AlertType.INFORMATION); 
            alert.setTitle("Seleccionar club");
            alert.setHeaderText("Club vacío");
            alert.setContentText("Por favor, seleccione un club válido");
            alert.show();
       }
       
       else if(notadouble < 0 || notadouble > 10){
           Alert alert = new Alert(AlertType.INFORMATION); 
            alert.setTitle("Error");
            alert.setHeaderText("Nota no válida");
            ...
            Alert alert = new Alert(AlertType.INFORMATION); 
            alert.setTitle("Error");
            alert.setHeaderText("Campo nombre vacío");
            alert.setContentText("Introduce un nombre");
            alert.show();
       }
    ...
    private boolean isDouble(String nota){
        try{
